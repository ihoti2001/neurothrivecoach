...
const Header: React.FC<{ logoUrl: string; navItems: NavItem[] }> = ({ logoUrl, navItems }) => {
    ...
    const renderNavItem = (item: NavItem, className: string) => {
        if (isInternal(item.url)) {
            return (
                <Link key={item.url} to={item.url} className={className}>
                    {item.label}
                </Link>
            );
        }
        return (
            <a key={item.url} href={item.url} className={className} rel="noreferrer" target="_blank">
                {item.label}
            </a>
        );
    };
                        ...
                        <div className="hidden md:flex flex-1 justify-end gap-8 items-center">
                            <nav className="flex items-center gap-8">
                                {navItems.map(item =>
                                    renderNavItem(
                                        item,
                                        `text-lg font-bold leading-normal transition-colors ${
                                            isInternal(item.url) && isActive(item.url)
                                                ? 'text-primary'
                                                : 'text-text-main dark:text-gray-300 hover:text-primary'
                                        }`
                                    )
                                )}
                            </nav>
...
const Footer: React.FC<{ logoUrl: string; settings: GlobalSettings }> = ({ logoUrl, settings }) => (
    <footer className="w-full bg-white dark:bg-[#162228] mt-auto border-t border-gray-100 dark:border-gray-800">
        <div className="w-full max-w-[1280px] px-4 md:px-10 py-12 mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                ...
                <div className="flex gap-4">
                    <a
                        ...
                        aria-label="TikTok"
                        rel="noreferrer"
                        target={settings.social_tiktok ? '_blank' : undefined}
                    >
            ...
            <div className="mt-8 text-center text-xs text-text-muted dark:text-gray-500">
                {settings.copyright_text || '(c) 2024 Neuro Thrive Coach. All rights reserved.'}
            </div>
        </div>
    </footer>
);
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        const fetchLayoutContent = async () => {
            ...
            if (settings?.fields) {
                setGlobalSettings(settings.fields);
            }

            const collections = await fetchCollections(
                ['navigation_menu', 'navigation_menu_item'],
                { include: 'navigation_menu_item' }
            );
            if (collections?.navigation_menu?.length) {
                const menu = collections.navigation_menu[0];
                const items = menu.navigation_menu_item || menu.items || [];
                const normalized = items
                    .map((item: any) => ({
                        label: item.label || item.name || '',
                        url: item.url || item.link || ''
                    }))
                    .filter((item: NavItem) => item.label && item.url);
                if (normalized.length) {
                    setNavItems(normalized);
                }
            } else if (collections?.navigation_menu_item?.length) {
                const normalized = collections.navigation_menu_item
                    .map((item: any) => ({
                        label: item.label || item.name || '',
                        url: item.url || item.link || ''
                    }))
    ...
    const logoUrl = globalSettings.logo_url || '/logo.png';
    ...
    return (
        <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden bg-background-light dark:bg-background-dark text-text-main dark:text-white">
            ...
            <Footer logoUrl={logoUrl} settings={globalSettings} />
        </div>
    );
};
